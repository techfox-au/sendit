...
Clone the repo `git clone git@github.com:techfox-au/sendit.git`, then `cd sendit\deploy` and edit the API settings contained within `docker-compose.yml`. Run the API with `docker compose up -d --build` Then you need to configure your webserver (I recommend NGINX) to serve `sendit\public`.
